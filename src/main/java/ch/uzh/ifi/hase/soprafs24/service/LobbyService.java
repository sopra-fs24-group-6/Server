...
public class LobbyService {
    ...
    @Autowired
    public LobbyService(SimpMessagingTemplate messagingTemplate, UserRepository userRepository, PlayerRepository playerRepository, @Qualifier("lobbyRepository") LobbyRepository lobbyRepository, ThemeRepository themeRepository) {
...
//    }
    ...
    public Lobby getLobbyByUsername(String username) {
        ...
        Player player = playerRepository.findByUsername(username).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Player with username " + username + " could not be found."));
    ...
    public Lobby getLobbyByUserId(Long userId) {
        ...
        Player player = playerRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Player with userId " + userId + " could not be found."));
...
//      }
    ...
    public List<Player> getPlayersById(Long lobbyId) {
        Lobby lobby = lobbyRepository.findById(lobbyId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Lobby with id " + lobbyId + " could not be found."));
    ...
    public Lobby updateLobby(Long lobbyId, Lobby newLobby) {
        ...
        if (newLobby.getPlayerLimit() >= targetLobby.getPlayerCount()) {
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Player limit should be less than current player counts");
        }
    ...
    public Lobby addPlayerToLobby(Long lobbyId, Long userId) {
        ...
        if (lobby.getPlayerCount() < lobby.getPlayerLimit()) {
            ...
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Lobby with id " + lobbyId + " is full.");
        }
    }
    ...
    public Lobby kickPlayerFromLobby(Long lobbyId, Long targetId, Long requesterId) {
        ...
        if (targetPlayer != null && targetPlayer.getLobby().getId().equals(lobbyId)) {
            ...
            if (lobby.getHost().getUserId().equals(requesterId)) {
                ...
                throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Kicking player is only allowed by the host.");
            }
        }
        else {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Target player with id" + targetId + " could not be found.");
        }
    }
    ...
    public void checkIfLobbyNameExists(String lobbyName) {
        lobbyRepository.findByName(lobbyName).ifPresent(existingLobby -> {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "Lobby with name: " + lobbyName + " already exists.");
        });
    }

    public Lobby findLobbyById(Long lobbyId) {
        return lobbyRepository.findById(lobbyId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Lobby with id " + lobbyId + " could not be found."));
    }

    public User findUserById(Long userId) {
        return userRepository.findById(userId).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User with id " + userId + " could not be found."));
    }
    ...
    public List<Theme> findThemesByNames(List<String> themeNames) {
        ...
        for (String themeName : themeNames) {
            Theme theme = themeRepository.findByName(themeName).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Theme not found with name: " + themeName));
            themes.add(theme);
        }
        return themes;
    }
    ...
    public Player createPlayerFromUser(User user, Boolean isHost) {
        ...
        if (user.getPlayer() != null) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "User with id " + user.getId() + " already joins a lobby.");
        }
        else {
            // create new player
